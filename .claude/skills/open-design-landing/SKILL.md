...
description: >
  ...
  surface. The agent fills a typed `inputs.json` — **TODO pending owner** (template not yet bundled) — from a brand brief,
  ...
  separate path can mirror the Astro marketing site in `apps/landing-page/` — **TODO pending owner** (Astro counterpart not yet bundled).
...
inputs:
  - id: brand
    ...
    schema_path: ./schema.ts#BrandBlock # TODO pending owner (schema not yet bundled)
  - id: nav
    label: Navigation links
    description: Up to 5 nav entries, each with optional count badge.
    schema_path: ./schema.ts#NavLink # TODO pending owner (schema not yet bundled)
  - id: hero
    label: Hero copy + 3 stat rings + 4-step index
    schema_path: ./schema.ts#HeroBlock # TODO pending owner (schema not yet bundled)
  - id: about
    label: Manifesto / about block
    schema_path: ./schema.ts#AboutBlock # TODO pending owner (schema not yet bundled)
  - id: capabilities
    label: 4 capability cards
    schema_path: ./schema.ts#CapabilitiesBlock # TODO pending owner (schema not yet bundled)
  - id: labs
    label: 5 lab cards + filter pills
    schema_path: ./schema.ts#LabsBlock # TODO pending owner (schema not yet bundled)
  - id: method
    label: 4 method steps with thumbnails
    schema_path: ./schema.ts#MethodBlock # TODO pending owner (schema not yet bundled)
  - id: work
    label: 2 selected-work cards on dark slab
    schema_path: ./schema.ts#WorkBlock # TODO pending owner (schema not yet bundled)
  - id: testimonial
    label: Pull quote + author + 5 partner glyphs
    schema_path: ./schema.ts#TestimonialBlock # TODO pending owner (schema not yet bundled)
  - id: cta
    label: Closing CTA + ribbon
    schema_path: ./schema.ts#CTABlock # TODO pending owner (schema not yet bundled)
  - id: footer
    label: Brand description + 4 link columns + mega kicker
    schema_path: ./schema.ts#FooterBlock # TODO pending owner (schema not yet bundled)
  - id: imagery
    label: Image strategy (generate / placeholder / bring-your-own)
    schema_path: ./schema.ts#ImageryConfig # TODO pending owner (schema not yet bundled)
parameters:
  output_format:
    type: enum
    ...
    description: >
      ...
      enum label for cloning the Astro-based `apps/landing-page/` — **TODO pending owner** — tree and
...
outputs:
  ...
  - path: <out>/nextjs/
    ...
    description: Astro static tree mirroring `apps/landing-page` — **TODO pending owner** (Astro counterpart not yet bundled; folder name is historical).
...
sibling [`open-design-landing-deck`](../open-design-landing-deck/) — **TODO pending owner** (slide-deck variant not yet bundled) — skill)
...
`inputs.json` — **TODO pending owner** (not yet bundled) from the user's brief; the composer turns that JSON +
the canonical [`styles.css` — **TODO pending owner** (not yet bundled)](./styles.css) into a deployable artifact.
...
brief in chunks; do **not** dump the entire `schema.ts` — **TODO pending owner** (not yet bundled) on the user.
Map their answers into `inputs.json` — **TODO pending owner** (template not yet bundled) — matching the typed shape.
...
Open `inputs.example.json` — **TODO pending owner** (example not yet bundled) — for a complete
...
When the companion bundle ships, run the future placeholder command:

```bash
# TODO pending owner: scripts/placeholder.ts is not yet bundled.
...
dimensions, and the prompt hint from `image-manifest.json` — **TODO pending owner**. The
composer's `<img src='./assets/hero.png'>` etc. just work.

#### `generate` — gpt-image-2 mode

When the companion bundle ships, run the future image generation command:

```bash
# TODO pending owner: scripts/imagegen.ts is not yet bundled.
...
from `inputs.json` — **TODO pending owner**) + **per-slot composition** (e.g. cropped plaster
...
Drop 16 PNGs matching `assets/image-manifest.json` — **TODO pending owner** — filenames at
`inputs.imagery.assets_path`. Done.

### 3. Compose the artifact

When the companion bundle ships, run the future composer command:

```bash
# TODO pending owner: scripts/compose.ts is not yet bundled.
npx tsx scripts/compose.ts <inputs.json> <out>/index.html
```

The composer reads `inputs.json` — **TODO pending owner** (not yet bundled) and `../styles.css` — **TODO pending owner** (not yet bundled), then writes one
...
- Inline IntersectionObserver script (mirrors
  `apps/landing-page/app/_components/reveal-root.tsx` — **TODO pending owner**).
- Inline Headroom nav script (mirrors `header.tsx` — **TODO pending owner**).
- Inline GitHub star-count fetcher (auto-detects from `brand.primary_url`).

### 4. (Optional) Mirror the deployable Astro site

For deployable production output, **fork the `apps/landing-page/` package** — TODO pending owner (not yet bundled):
copy it into your workspace, align `app/page.tsx` — **TODO pending owner** (not yet bundled) with content
from your `inputs.json` — **TODO pending owner** (not yet bundled), and copy your `<out>/assets/*.png` into the
paths expected by `app/image-assets.ts` — **TODO pending owner** — / R2 URLs. Build with
...
> `apps/landing-page/` — **TODO pending owner** — tree from `inputs.json` in one command. Until
...
├── README.md                # TODO pending owner (not yet bundled)
├── schema.ts                # TODO pending owner (not yet bundled)
├── styles.css               # TODO pending owner (not yet bundled)
├── inputs.example.json      # TODO pending owner (not yet bundled)
├── example.html             # TODO pending owner (not yet bundled)
├── scripts/                 # TODO pending owner (not yet bundled)
│   ├── compose.ts           # TODO pending owner
│   ├── imagegen.ts          # TODO pending owner
│   └── placeholder.ts       # TODO pending owner
└── assets/                  # TODO pending owner (not yet bundled)
    ├── *.png                # TODO pending owner
    ├── image-manifest.json  # TODO pending owner
    └── imagegen-prompts.md  # TODO pending owner
```
...
- **Do not** invent new colors or typefaces. Tokens live in
  `design-systems/atelier-zero/DESIGN.md` — **TODO pending owner** (not yet bundled); extend the design system
...
- **Do not** add a separate stylesheet file for the Astro landing-page
  fork; copy `styles.css` — **TODO pending owner** (not yet bundled) verbatim into `app/globals.css` — **TODO pending owner** (not yet bundled) so visual parity
  stays one-to-one.

## See also

- [`design-systems/atelier-zero/DESIGN.md` — **TODO pending owner** (not yet bundled)](../../design-systems/atelier-zero/DESIGN.md) — token spec.
- [`apps/landing-page/` — **TODO pending owner** (not yet bundled)](../../apps/landing-page/) — deployable Astro static counterpart.
- [`skills/open-design-landing-deck/` — **TODO pending owner** (not yet bundled)](../open-design-landing-deck/) — sibling slides skill that reuses this design system.
